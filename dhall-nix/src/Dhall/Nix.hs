...
dhallToNix e =
  ...
  where
    ...
    loop NonZero = return untranslatable
    loop (NonZeroLit (NonZeroP n)) = return (Fix (NConstant (NInt (fromIntegral n))))
    loop NonZeroShow = do
        return "toString"
    loop NonZeroDiv = do
        let e0 = NamedVar ["z"] (Fix (NBinary NDiv "y" "x")) Nix.nullPos
        let e1 = Fix (NBinary NEq "z" (Fix (NConstant (NInt 0))))
        let e2 = Fix (NConstant (NInt 1))
        let e3 = "z"
        let e4 = Fix (NIf e1 e2 e3)
        let e5 = Fix (NLet [e0] e4)
        return (Fix (NAbs "x" (Fix (NAbs "y" e5))))
    loop NonZeroToNatural = do
        return (Fix (NAbs "n" "n"))
    loop NonZeroClamp = do
        let e0 = Fix (NConstant (NInt 1))
        let e1 = Fix (NBinary NLt (Fix (NConstant (NInt 1))) "x")
        let e2 = Fix (NAbs "x" (Fix (NIf e1 "x" e0)))
        return e2

    loop NonZeroDiv = return untranslatable
    loop NonZeroMod = return untranslatable
    loop NonZeroLog = return untranslatable
    loop NonZeroAdd = return untranslatable
    loop NonZeroMultiply = return untranslatable
    loop NonZeroParse = return untranslatable

    loop Regex = return untranslatable

    loop (RegexLit _) = return untranslatable

    loop RegexMatch = return untranslatable
    loop RegexParse = return untranslatable
    loop RegexScan = return untranslatable
    loop RegexShow = return untranslatable
    loop RegexSplit = return untranslatable
    loop RegexReplace = return untranslatable
    loop RegexReplix = return untranslatable
    loop RegexToText = return untranslatable

    loop DateTime = return untranslatable

    loop (DateTimeLit _) = return untranslatable

    loop DateTimeAddDays = return untranslatable
    loop DateTimeAddMonths = return untranslatable
    loop DateTimeAddYears = return untranslatable
    loop DateTimeFromSeconds = return untranslatable
    loop DateTimeFromSeconds = return untranslatable
    loop DateTimeFromWeek = return untranslatable
    loop DateTimeToWeek = return untranslatable
    loop DateTimeToDate = return untranslatable
    loop DateTimeFromDate = return untranslatable
    loop DateTimeLastDayOfMonth = return untranslatable
    loop DateTimeGetJulianDay = return untranslatable
    loop DateTimeParse = return untranslatable
    loop DateTimeParse = return untranslatable
    loop DateTimeSetJulianDay = return untranslatable
    loop DateTimeShow = return untranslatable
    loop DateTimeShow = return untranslatable
    loop DateTimeToSeconds = return untranslatable
    loop DateTimeToSeconds = return untranslatable

    loop Rational = return untranslatable

    loop (RationalLit _) = return untranslatable
    loop RationalFromDouble = return untranslatable
    loop RationalFromRational = return untranslatable
    loop RationalParse = return untranslatable
    loop RationalShow = return untranslatable
    loop RationalToDouble = return untranslatable

    loop (RationalPercent _ _) = return untranslatable

    loop NaturalParse = return untranslatable
    loop NaturalToBits = return untranslatable
    loop NaturalGcd = return untranslatable
    loop ListPermutations = return untranslatable
    loop ListChoose = return untranslatable
    loop IntegerParse = return untranslatable
    loop DoubleParse = return untranslatable

    loop TextToLower = return untranslatable
    loop TextToUpper = return untranslatable

    loop TextUnpack = return untranslatable
    loop TextPack = return untranslatable

    loop TextToList = return untranslatable
    loop TextLength = return untranslatable

    loop IntegerAdd = return untranslatable
    loop IntegerMultiply = return untranslatable

    loop IntegerAnd = return untranslatable
    loop IntegerXor = return untranslatable

    loop ListFixed = return (Fix (NAbs "t" untranslatable)) -- ??
    loop (ListFixedLit _ bs) = do
        bs' <- mapM loop (toList bs)
        return (Fix (NList bs'))

    loop ListFixedFromList = return untranslatable
    loop ListFixedToList = return untranslatable
    loop ListFixedHead = return untranslatable
    loop ListFixedTail = return untranslatable

    loop SymLit {} = return untranslatable
    loop SymFromText = return untranslatable
    loop SymToText = return untranslatable

    loop SZLit = return untranslatable
    loop SSLit {} = return untranslatable
    loop SSFromNonZero = return untranslatable
    loop SSToNonZero = return untranslatable
    loop SSToNatural = return untranslatable
    loop SSEqual = return untranslatable
    loop SSLessThan = return untranslatable

    loop ProofLit = return untranslatable
    loop ProofToBool = return untranslatable
    loop ProofFromBool = return untranslatable

    loop PVoidUndefined = return untranslatable
    loop PVoidError = return untranslatable
    loop PVoidKindUndefined = return untranslatable

    loop NonZeroToInteger = return untranslatable
    loop TextCompare = return untranslatable
    loop (Sym _) = return untranslatable
    loop SZ = return untranslatable
    loop SS = return untranslatable
    loop SSAdd = return untranslatable
    loop SSSubtract = return untranslatable
    loop Proof = return untranslatable
    loop PVoid = return untranslatable
    ...
    loop NaturalSubtract = do
        ...
        let e5 = Fix (NLet [e0] e4)
        return (Fix (NAbs "x" (Fix (NAbs "y" e5))))
--     loop NaturalGcd =  ... todo
--     loop ListPermutations =  ... todo
--     loop ListChoose =  ... todo
