...
>>> input nonzero "!42"
NonZeroC {unNonZero = 42}
-}
nonzero :: Decoder NonZero
nonzero = Decoder {..}
  where
    extract (NonZeroLit n) = pure n
    extract  expr          = typeError expected expr

    expected = pure NonZero

{-| Decode a `DateTime`

>>> input datetime "^2001-02-03T12:22:33"
DateTimeC {unDateTime = 981202953}
-}
datetime :: Decoder DateTime
datetime = Decoder {..}
  where
    extract (DateTimeLit n) = pure n
    extract  expr           = typeError expected expr

    expected = pure DateTime

{-| Decode a `DateTime`

>>> input utctime "^2001-02-03T12:22:33"
2001-02-03 12:22:33 UTC
-}
utctime :: Decoder UTCTime
utctime = Decoder {..}
  where
    extract (DateTimeLit n) = pure (Core.dateTimeToUtc n)
    extract  expr           = typeError expected expr
...
rational = Decoder {..}
  where
    extract (RationalLit t) = pure t
    extract  expr           = typeError expected expr

    expected = pure Rational

{-| Decode an `Ordering`

>>> input ordering "< LT | EQ | GT>.EQ"
EQ
-}

ordering :: Decoder Ordering
ordering =
  union
    (constructor "LT" (LT <$ unit)
  <> constructor "EQ" (EQ <$ unit)
  <> constructor "GT" (GT <$ unit)
  )
...
>>> input (thesed natural bool) "let f = \\(a : Type) -> \\(b : Type) -> < This : a | That : b | These : { _1 : a, _2 : b } > in (f Natural Bool).This 10"
This 10

>>> input (thesed natural bool) "let f = \\(a : Type) -> \\(b : Type) -> < This : a | That : b | These : { _1 : a, _2 : b } > in (f Natural Bool).These { _1 = 10, _2 = False }"
These 10 False
-}
thesed :: forall a b . Decoder a -> Decoder b -> Decoder (These a b)
thesed l r =
  union
    (constructor "This" (This <$> l)
  <> constructor "That" (That <$> r)
  <> constructor "These" (uncurry These <$> pair l r))

{-| Decode a `Either`

>>> input (eitherd natural bool) "let f = \\(a : Type) -> \\(b : Type) -> < Left : a | Right : b > in (f Natural Bool).Left 10"
Left 10
-}
eitherd :: forall a b . Decoder a -> Decoder b -> Decoder (Either a b)
eitherd l r =
  union
    (constructor "Left" (Left <$> l)
  <> constructor "Right" (Right <$> r))
...
>>> input (nonEmpty (Core.NonZeroC 3) natural) "~[1, 2, 3]"
1 :| [2,3]
-}
nonEmpty :: NonZero -> Decoder a -> Decoder (NonEmpty a)
nonEmpty nz (Decoder extractIn expectedIn) = Decoder extractOut expectedOut
  where
    extractOut (ListFixedLit e es) = (:|) <$> extractIn e <*> (Data.Foldable.toList <$> traverse extractIn es)
    extractOut expr                = typeError expectedOut expr

    expectedOut = App ( App ListFixed (Core.makeSSExpr nz) ) <$> expectedIn
...
{-| Given a triple of `Decoder`s, decode a 3-tuple-record into their pairing.

>>> input (triple natural bool double) "{ _1 = 42, _2 = False, _3 = -4.4 }"
(42,False,-4.4)
-}
triple :: Decoder a -> Decoder b -> Decoder c -> Decoder (a, b, c)
triple l m r = Decoder extractOut expectedOut
  where
    extractOut expr@(RecordLit fields) =
      (,,) <$> Data.Maybe.maybe (typeError expectedOut expr) (extract l)
                  (Core.recordFieldValue <$> Dhall.Map.lookup "_1" fields)
           <*> Data.Maybe.maybe (typeError expectedOut expr) (extract m)
                  (Core.recordFieldValue <$> Dhall.Map.lookup "_2" fields)
           <*> Data.Maybe.maybe (typeError expectedOut expr) (extract r)
                  (Core.recordFieldValue <$> Dhall.Map.lookup "_3" fields)
    extractOut expr = typeError expectedOut expr

    expectedOut = do
        l' <- Core.makeRecordField <$> expected l
        m' <- Core.makeRecordField <$> expected m
        r' <- Core.makeRecordField <$> expected r
        pure $ Record $ Dhall.Map.fromList [("_1", l'), ("_2", m'), ("_3", r')]
...
instance FromDhall NonZero where
    autoWith _ = nonzero

instance FromDhall DateTime where
    autoWith _ = datetime

instance FromDhall UTCTime where
    autoWith _ = utctime

instance FromDhall Regex where
    autoWith _ = regex

instance FromDhall Rational where
  autoWith _i = rational
...
instance FromDhall Ordering where
    autoWith _ = ordering
...
instance (FromDhall a, FromDhall b) => FromDhall (These a b) where
    autoWith opts = thesed (autoWith opts) (autoWith opts)

instance (FromDhall a, FromDhall b) => FromDhall (Either a b) where
    autoWith opts = eitherd (autoWith opts) (autoWith opts)
...
instance ToDhall NonZero where
    injectWith _ = Encoder {..}
      where
        embed = NonZeroLit
...
instance ToDhall DateTime where
    injectWith _ = Encoder {..}
      where
        embed = DateTimeLit
...
instance ToDhall UTCTime where
    injectWith _ = Encoder {..}
      where
        embed = DateTimeLit . Core.utcToDateTime

        declared = DateTime

instance ToDhall Regex where
    injectWith _ = Encoder {..}
      where
        embed = RegexLit
...
instance ToDhall Rational where
    injectWith _ = Encoder {..}
      where
        embed = RationalLit
...
instance ToDhall Rational where
  injectWith _i =
    let e = divide (\r ->
                     (fromIntegral (Data.Ratio.numerator r), mkNonZeroUnsafe (fromIntegral ((Data.Ratio.denominator r))))
                   )
                   (encodeField @Integer "num")
                   (encodeField @NonZero "den")
    in recordEncoder e
-}
...
instance (ToDhall a, ToDhall b) => ToDhall (These a b) where
  injectWith o = adapt >$< unionEncoder
   (   encodeConstructorWith "This" (injectWith o)
   >|< encodeConstructorWith "That" (injectWith o)
   >|< encodeConstructorWith "These" (injectWith o)
   )
   where
    adapt (This a) = Left a
    adapt (That b) = Right (Left b)
    adapt (These a b) = Right (Right (a,b))

instance (ToDhall a, ToDhall b) => ToDhall (Either a b) where
  injectWith o = unionEncoder
   (   encodeConstructorWith "Left" (injectWith o)
   >|< encodeConstructorWith "Right" (injectWith o)
   )

instance ToDhall Ordering where
  injectWith o = adapt >$< unionEncoder
   (   encodeConstructorWith "LT" (injectWith o)
   >|< encodeConstructorWith "EQ" (injectWith o)
   >|< encodeConstructorWith "GT" (injectWith o)
   )
   where
    adapt LT = Left ()
    adapt EQ = Right (Left ())
    adapt GT = Right (Right ())
...
instance (ToDhall a, ToDhall b, ToDhall c) => ToDhall (a, b, c)
