    ...
    loop ctx@Ctx{..} expression = case expression of
        ...
        BoolIf t l r -> do
            ...
            case tL' of
                VConst Type ->
                    return ()
                VConst Kind -> -- trace "TypeCheck: allowing Kind through on lhs" $
                               return ()
                -- VConst Sort -> trace "TypeCheck: allowing Sort through on lhs" $ return ()
            ...
            case tR' of
                VConst Type ->
                    return ()
                VConst Kind -> -- trace "TypeCheck: allowing Kind through on rhs" $
                               return ()
                --VConst Sort -> trace "TypeCheck: allowing Sort through on rhs" $ return ()
        ...
        NonZeroShow -> do
            return (VNonZero ~> VText)

        NonZeroClamp -> do
            return (VNatural ~> VNonZero)

        NonZeroDiv -> do
            return (VInteger ~> VNonZero ~> VInteger)

        NonZeroMod -> do
            return (VInteger ~> VNonZero ~> VInteger)

        NonZeroLog -> do
            return (VNonZero ~> VNonZero ~> VNatural)

        NonZeroAdd -> do
            return (VNonZero ~> VNonZero ~> VNonZero)

        NonZeroMultiply -> do
            return (VNonZero ~> VNonZero ~> VNonZero)

        NonZeroToNatural -> do
            return (VNonZero ~> VNatural)

        NonZeroToInteger -> do
            return (VNonZero ~> VInteger)

        NonZeroParse -> do
            return (VText ~> VOptional VNonZero)
        ...
        DateTimeShow -> do
            return (VDateTime ~> VText)

        DateTimeParse -> do
            return (VText ~> VOptional VDateTime)

        DateTimeFromSeconds -> do
            return (VInteger ~> VDateTime)

        DateTimeToSeconds -> do
            return (VDateTime ~> VInteger)

        DateTimeAddYears -> do
            return (VInteger ~> VDateTime ~> VDateTime)

        DateTimeAddMonths -> do
            return (VInteger ~> VDateTime ~> VDateTime)
        ...
        DateTimeFromWeek -> do
            return (VInteger ~> VNonZero ~> VNonZero ~> VDateTime)

        DateTimeToWeek -> do
            return (VDateTime ~> (VRecord $ Dhall.Map.unorderedFromList [("year",VInteger), ("week", VNonZero), ("day", VNatural)] ))

        DateTimeToDate -> do
            return (VInteger ~> VNonZero ~> VNonZero ~> VNatural ~> VNatural ~> VNatural ~> VDateTime)

        DateTimeFromDate -> do
            return (VDateTime ~> (VRecord $ Dhall.Map.unorderedFromList [("year",VInteger), ("month", VNonZero), ("day", VNonZero), ("hour", VNatural), ("minute", VNatural), ("second", VNatural)] ))

        DateTimeLastDayOfMonth -> do
            return (VInteger ~> VNonZero ~> VNonZero)


        DateTimeGetJulianDay -> do
            return (VDateTime ~> VNonZero)

        DateTimeSetJulianDay -> do
            return (VInteger ~> VNonZero ~> VDateTime)
        ...
        RegexMatch -> do
            return (VRegex ~> VText ~> VBool)
        RegexScan -> do
            return (VRegex ~> VText ~> VNonZero ~> VList (VRecord $ Dhall.Map.unorderedFromList [("_1",VText), ("_2", VList VText)] ) )
        RegexSplit -> do
            return (VRegex ~> VText ~> VNonZero ~> VList VText)
        RegexReplace -> do
            return (VRegex ~> VText ~> VText ~> VText)
        RegexReplix -> do
            -- VPi is forall
            -- Regex -> (forall(xs : Text) -> Text) -> Text -> Text
            return (VRegex ~> VPi VText (Eval.Closure "xs" Empty Dhall.Core.Text) ~> VText ~> VText)
            -- VLam is lambda
            -- Regex -> (\(xs : Text) -> Text) -> Text -> Text
--            return (VRegex ~> VLam VText (Eval.Closure "xs" Empty Dhall.Core.Text) ~> VText ~> VText)

        RegexParse -> do
            return (VText ~> VOptional VRegex)
        RegexToText -> do
            return (VRegex ~> VText)
        ...
        RationalLit _ -> do
            return VRational
        RationalShow -> do
            return (VRational ~> VText)
        RationalFromDouble -> do
            return (VDouble ~> VDouble ~> VRational)
        RationalToDouble -> do
            return (VRational ~> VDouble)
        RationalFromRational -> do
            return (VRational ~> VRecord (Dhall.Map.unorderedFromList [("num",VInteger), ("den", VNonZero)] ))

        RationalPercent l r -> do
            tl' <- loop ctx l

            case tl' of
                VInteger -> return ()
                _        -> die (CantRationalInteger l (quote names tl'))

            tr' <- loop ctx r

            case tr' of
                VNonZero -> return ()
                _        -> die (CantRationalNonZero r (quote names tr'))

            return VRational

        RationalParse -> do
            return (VText ~> VOptional VRational)
        ...
        NaturalParse -> do
            return (VText ~> VOptional VNatural)

        NaturalToBits -> do
            return (VNatural ~> VList VBool)
        ...
        IntegerAdd -> do
            return (VInteger ~> VInteger ~> VInteger)
        ...
        IntegerParse -> do
            return (VText ~> VOptional VInteger)
        ...
        DoubleParse -> do
            return (VText ~> VOptional VDouble)
        ...
        TextUnpack -> do
            return (VText ~> VList VNatural)

        TextPack -> do
            return (VList VNatural ~> VText)

        TextToList -> do
            return (VText ~> VList VText)

        TextLength -> do
            return (VText ~> VNatural)

        TextCompare -> do
            return (VBool ~> VText ~> VText ~> VNatural)
        ...
        ListLit Nothing ts₀ ->
            case Data.Sequence.viewl ts₀ of
                t₀ :< ts₁ -> do
                    ...
                    case tT₀' of
                        VConst Type -> return ()
                        VConst Kind -> -- trace "TypeCheck: allowing Kind through on list" $
                                       return ()
                        --VConst Sort -> trace "TypeCheck: allowing Sort through on list" $ return ()
        ...
        ListPermutations -> do
            return (VHPi "a" (VConst Type) (\a -> VList a ~> VList (VList a)))

        ListChoose -> do
            return (VHPi "a" (VConst Type) (\a -> VNatural ~> VList a ~> VList (VList a)))

        ListFixed -> do
            return (VConst Type ~> VConst Type ~> VConst Type)

        ListFixedLit t0 ts1 -> do
                    _T0' <- loop ctx t0

                    let _T0'' = quote names _T0'

                    tT0' <- loop ctx _T0''

                    case tT0' of
                        VConst Type -> return ()
                        VConst Kind -> -- trace "TypeCheck: allowing Kind through on listfixed" $
                                       return ()
                        --VConst Sort -> trace "TypeCheck: allowing Sort through on listfixed" $ return ()
                        _           -> die (InvalidListFixedType (App (App ListFixed SS) _T0''))

                    let process i t1 = do
                            _T1' <- loop ctx t1

                            if Eval.conv values _T0' _T1'
                                then do
                                    return ()

                                else do
                                    let _T0'' = quote names _T0'
                                    let _T1'' = quote names _T1'

                                    -- Carefully note that we don't use `die`
                                    -- here so that the source span is narrowed
                                    -- to just the offending element
                                    let err = MismatchedListFixedElements (i+1) _T0'' t1 _T1''

                                    Left (TypeError context t1 err)

                    traverseWithIndex_ process ts1
                    let vss = foldr (const VSS) VSZ [1..length ts1]

                    return (VListFixed vss _T0')

        ListFixedFromList -> do
            return (VHPi "n" (VConst Type) (\n -> VHPi "a" (VConst Type) (\a -> VHPi "def" a (\_def -> VList a ~> VListFixed n a))))

        ListFixedToList -> do
            return (VHPi "n" (VConst Type) (\n -> VHPi "a" (VConst Type) (\a -> VListFixed n a ~> VList a)))

        ListFixedHead -> do
            return (VHPi "n" (VConst Type) (\n -> VHPi "a" (VConst Type) (\a -> VListFixed n a ~> a)))
        ...
        SymLit t -> do
            return (VSym t)
        SymFromText -> do
            return (VText ~> VConst Type)
        SymToText -> do
            return (VConst Type ~> VText)
        ...
        SSFromNonZero -> do
            return (VNonZero ~> VConst Type)
        SSToNonZero -> do
            return (VConst Type ~> VNonZero)
        SSToNatural -> do
            return (VConst Type ~> VNatural)
        SSAdd -> do
            return (VConst Type ~> VConst Type ~> VConst Type)
        SSSubtract -> do
            return (VConst Type ~> VConst Type ~> VConst Type)
...
-- once you do this it is a different type and will not match!
        Proof -> do
            return (VConst Type ~> VConst Type)
        ProofLit -> do
            return (VProof (VConst Type))
        ProofToBool -> do
            return (VConst Type ~> VBool)
        ProofFromBool -> do
            return (VConst Type ~> VBool ~> VConst Type)

        PVoid -> do
            return (VConst Type ~> VConst Type)
        PVoidUndefined -> do
            return (VHPi "A" (VConst Type) (\_A -> _A))
        PVoidError -> do
            return (VConst Type ~> VHPi "A" (VConst Type) (\_A -> _A))
        PVoidKindUndefined -> do
            return (VHPi "A" (VConst Kind) (\_A -> _A))
        ...
        Some a -> do
            ...
            case tA' of
                VConst Type -> return ()
                VConst Kind -> -- trace "TypeCheck: allowing Kind through on some" $
                               return ()
                --VConst Sort -> trace "TypeCheck: allowing Sort through on some" $ return ()
        ...
        ToMap e mT₁ -> do
            ...
            case tE' of
                VConst Type -> return ()
                VConst Kind -> -- trace "TypeCheck: allowing Kind through on ToMap" $
                               return ()
              --VConst Sort -> trace "TypeCheck: allowing Sort through on ToMap" $ return ()
        ...
        Equivalent x y -> do
            ...
            case tA₀' of
                VConst Type -> return ()
                VConst Kind -> -- trace "TypeCheck: allowing Kind through on Equivalent" $
                               return ()
                --VConst Sort -> trace "TypeCheck: allowing Sort through on Equivalent" $ return ()
...
data TypeMessage s a
    ...
    | Untyped
    | MissingListType
    | MissingListFixedType
    | MismatchedListElements Int (Expr s a) (Expr s a) (Expr s a)
    | MismatchedListFixedElements Int (Expr s a) (Expr s a) (Expr s a)
    | InvalidListElement Int (Expr s a) (Expr s a) (Expr s a)
    | InvalidListType (Expr s a)
    | InvalidListFixedType (Expr s a)
    | ListLitInvariant
    | ListFixedLitInvariant
    ...
    | CantRationalInteger (Expr s a) (Expr s a)
    | CantRationalNonZero (Expr s a) (Expr s a)
    deriving (Show)
...
prettyTypeMessage (UnboundVariable x) = ErrorMessages {..}
  ...
  where
    ...
    long =
        ...
        \                                                                                \n\
        \    ┌─────────────────────────────────────────────────────────┐                 \n\
        \    │ λ(empty : Bool) → if empty then \"Empty\" else \"Full\" │                 \n\
        \    └─────────────────────────────────────────────────────────┘                 \n\
        \                           ⇧                                                    \n\
        \                           Typo                                                 \n\
        \                                                                                \n\
...
prettyTypeMessage (ListFixedLitInvariant) = ErrorMessages {..}
  where
    short = "Internal error: A non-empty fixed list literal violated an internal invariant"
...
prettyTypeMessage (InvalidListFixedType expr0) = ErrorMessages {..}
  where
    short = "Invalid type for fixed ❰List❱"
...
prettyTypeMessage MissingListFixedType = do
    ErrorMessages {..}
  where
    short = "An empty fixed list requires a type annotation ... todo"
...
prettyTypeMessage (MismatchedListFixedElements i expr0 _expr1 expr2) =
    ErrorMessages {..}
  where
    short = "List elements for fixed should all have the same type\n"
        <>  "\n"
        <>  Dhall.Diff.doc (Dhall.Diff.diffNormalized expr0 expr2)
...
prettyTypeMessage (CantRationalInteger expr0 expr1) =
        buildRationalOperator "%" expr0 expr1

prettyTypeMessage (CantRationalNonZero expr0 expr1) =
        buildRationalOperator "%" expr0 expr1
...
buildRationalOperator :: Pretty a => Text -> Expr s a -> Expr s a -> ErrorMessages
buildRationalOperator operator expr0 expr1 = ErrorMessages {..}
  where
    short = "❰" <> txt2 <> "❱ only works on an ❰Integer❱ and a ❰NonZero❱"

    long =
        "Explanation: The ❰" <> txt2 <> "❱ operator expects two arguments where the left has type ❰Integer❱ and the right has type ❰NonZero❱\n\
        \                                                                                \n\
        \For example, this is a valid use of ❰" <> txt2 <> "❱:                           \n\
        \                                                                                \n\
        \                                                                                \n\
        ...
        \" <> txt0 <> "\n\
        \                                                                                \n\
        \... which does not have type ❰Integer❱ or ❰NonZero❱ but instead has type:       \n\
        \                                                                                \n\
        \" <> txt1 <> "\n"
      where
        txt0 = insert expr0
...
messageExpressions f m = case m of
    ...
    MissingListFixedType ->
        pure MissingListFixedType
    MismatchedListElements a b c d ->
        MismatchedListElements <$> pure a <*> f b <*> f c <*> f d
    MismatchedListFixedElements a b c d ->
        MismatchedListFixedElements <$> pure a <*> f b <*> f c <*> f d
    InvalidListElement a b c d ->
        InvalidListElement <$> pure a <*> f b <*> f c <*> f d
    InvalidListType a ->
        InvalidListType <$> f a
    InvalidListFixedType a ->
        InvalidListFixedType <$> f a
    ListLitInvariant ->
        pure ListLitInvariant
    ListFixedLitInvariant ->
        pure ListFixedLitInvariant
    InvalidSome a b c ->
    ...
    CantRationalInteger a b ->
        CantRationalInteger <$> f a <*> f b
    CantRationalNonZero a b ->
        CantRationalNonZero <$> f a <*> f b
