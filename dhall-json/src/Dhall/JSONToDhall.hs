...
dhallFromJSON (Conversion {..}) expressionType =
  ...
  where
    ...
    -- number ~> NonZero
    loop jsonPath D.NonZero (Aeson.Number x)
        | Right n <- floatingOrInteger x :: Either Double Integer
        , n >= 0
        = Right (D.NonZeroLit (mkNonZeroUnsafe (fromInteger n)))
        | otherwise
        = Left (Mismatch D.NonZero (Aeson.Number x) jsonPath)
