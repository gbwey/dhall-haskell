...
import Dhall.Core (Binding(..), DhallDouble(..), Expr, dateTimeToUtc)
...
import Dhall (pattern NonZeroP, DateTime(..))
...
dhallToJSON e0 = loop (Core.alphaNormalize (Core.normalize e0))
  where
    loop e = case e of
        ...
        Core.NonZeroLit (NonZeroP a) -> return (toJSON a)
        Core.DateTimeLit (DateTimeC a) -> return (toJSON (dateTimeToUtc (fromIntegral a)))
        Core.RegexLit a -> do
            return (toJSON a)
        Core.RationalLit r -> return (toJSON (Data.Ratio.approxRational r 0))
...
convertToHomogeneousMaps (Conversion {..}) e0 = loop (Core.normalize e0)
  where
    loop e = case e of
        ...
        Core.NonZeroMultiply ->
            Core.NonZeroMultiply

        Core.NonZeroToNatural ->
            Core.NonZeroToNatural

        Core.NonZeroToInteger ->
            Core.NonZeroToInteger
        ...
        Core.DateTimeFromSeconds ->
            Core.DateTimeFromSeconds

        Core.DateTimeToSeconds ->
            Core.DateTimeToSeconds

        Core.DateTimeAddYears ->
            Core.DateTimeAddYears

        Core.DateTimeAddMonths ->
            Core.DateTimeAddMonths

        Core.DateTimeAddDays ->
            Core.DateTimeAddDays

        Core.DateTimeFromWeek ->
            Core.DateTimeFromWeek
        ...
        Core.DateTimeFromDate ->
            Core.DateTimeFromDate

        Core.DateTimeLastDayOfMonth ->
            Core.DateTimeLastDayOfMonth

        Core.DateTimeGetJulianDay ->
            Core.DateTimeGetJulianDay

        Core.DateTimeSetJulianDay ->
            Core.DateTimeSetJulianDay
        ...
        Core.RationalFromDouble ->
            Core.RationalFromDouble

        Core.RationalToDouble ->
            Core.RationalToDouble

        Core.RationalFromRational ->
            Core.RationalFromRational

        Core.RationalPercent a b ->
            Core.RationalPercent a' b'
          where
            a' = loop a
            b' = loop b
        ...
        Core.ListFixedLit b c -> loop (Core.ListLit Nothing (b Data.Sequence.:<| c))

        Core.SymLit t -> Core.SymLit t

        Core.SZLit -> Core.SZLit

        Core.SSLit a -> Core.SSLit (loop a)

        Core.ProofLit -> Core.ProofLit
        ...
        Core.ListFixedFromList ->
            Core.ListFixedFromList

        Core.ListFixedToList ->
            Core.ListFixedToList
