...
import Dhall (pattern NonZeroP, DateTime(..), Regex(..))
...
dhallToStatement expr0 var0 = go (Dhall.Core.normalize expr0)
  where
    ...
    go (NonZeroLit (NonZeroP a)) = do
        go (IntegerLit (fromIntegral a))
    go (DateTimeLit (DateTimeC a)) = do
        go (IntegerLit a)
    go (RegexLit a) = do
        e <- first adapt (dhallToExpression (RegexLit a))
    ...
    go (ListLit _ bs) = do
        ...
        let bytes
                ...
                <>  ")"
        return bytes
    go (ListFixedLit b bs) = do
        bs' <- first adapt (mapM dhallToExpression (b Data.Sequence.:<| bs))
        let bytes
                =   "declare -r -a "
                <>  var
                <>  "=("
    ...
    go e@(NonZero            ) = Left (UnsupportedStatement e)
    go e@(NonZeroShow        ) = Left (UnsupportedStatement e)
    go e@(NonZeroClamp       ) = Left (UnsupportedStatement e)
    go e@(NonZeroDiv         ) = Left (UnsupportedStatement e)
    go e@(NonZeroMod         ) = Left (UnsupportedStatement e)
    go e@(NonZeroLog         ) = Left (UnsupportedStatement e)
    go e@(NonZeroAdd         ) = Left (UnsupportedStatement e)
    go e@(NonZeroMultiply         ) = Left (UnsupportedStatement e)
    go e@(NonZeroToNatural   ) = Left (UnsupportedStatement e)
    go e@(NonZeroToInteger   ) = Left (UnsupportedStatement e)
    go e@(NonZeroParse       ) = Left (UnsupportedStatement e)

    go e@(DateTime           ) = Left (UnsupportedStatement e)
    go e@(DateTimeShow       ) = Left (UnsupportedStatement e)
    go e@(DateTimeParse      ) = Left (UnsupportedStatement e)
    go e@(DateTimeFromSeconds) = Left (UnsupportedStatement e)
    go e@(DateTimeToSeconds  ) = Left (UnsupportedStatement e)

    go e@(DateTimeAddYears   ) = Left (UnsupportedStatement e)
    go e@(DateTimeAddMonths  ) = Left (UnsupportedStatement e)
    go e@(DateTimeAddDays    ) = Left (UnsupportedStatement e)
    go e@(DateTimeFromWeek ) = Left (UnsupportedStatement e)
    go e@(DateTimeToWeek    ) = Left (UnsupportedStatement e)
    go e@(DateTimeToDate    ) = Left (UnsupportedStatement e)
    go e@(DateTimeFromDate    ) = Left (UnsupportedStatement e)
    go e@(DateTimeLastDayOfMonth    ) = Left (UnsupportedStatement e)

    go e@(DateTimeGetJulianDay) = Left (UnsupportedStatement e)
    go e@(DateTimeSetJulianDay) = Left (UnsupportedStatement e)

    go e@(Regex              ) = Left (UnsupportedStatement e)
    go e@(RegexShow          ) = Left (UnsupportedStatement e)
    go e@(RegexMatch         ) = Left (UnsupportedStatement e)
    go e@(RegexScan          ) = Left (UnsupportedStatement e)
    go e@(RegexSplit         ) = Left (UnsupportedStatement e)
    go e@(RegexReplace       ) = Left (UnsupportedStatement e)
    go e@(RegexReplix        ) = Left (UnsupportedStatement e)
    go e@(RegexParse         ) = Left (UnsupportedStatement e)
    go e@(RegexToText        ) = Left (UnsupportedStatement e)

    go e@(Rational           ) = Left (UnsupportedStatement e)
    go e@(RationalShow       ) = Left (UnsupportedStatement e)
    go e@(RationalFromDouble ) = Left (UnsupportedStatement e)
    go e@(RationalToDouble   ) = Left (UnsupportedStatement e)
    go e@(RationalFromRational ) = Left (UnsupportedStatement e)
    go e@(RationalPercent  {}) = Left (UnsupportedStatement e)
    go e@(RationalParse      ) = Left (UnsupportedStatement e)
    go e@(RationalLit      {}) = Left (UnsupportedStatement e)
    ...
    go e@(NaturalParse       ) = Left (UnsupportedStatement e)
    go e@(NaturalToBits      ) = Left (UnsupportedStatement e)
    go e@(NaturalSubtract    ) = Left (UnsupportedStatement e)
    go e@(NaturalGcd         ) = Left (UnsupportedStatement e)
    ...
    go e@(IntegerAdd         ) = Left (UnsupportedStatement e)
    go e@(IntegerMultiply    ) = Left (UnsupportedStatement e)
    go e@(IntegerAnd         ) = Left (UnsupportedStatement e)
    go e@(IntegerXor         ) = Left (UnsupportedStatement e)
    ...
    go e@(IntegerParse       ) = Left (UnsupportedStatement e)
    ...
    go e@(DoubleParse        ) = Left (UnsupportedStatement e)
    ...
    go e@(TextToLower      {}) = Left (UnsupportedStatement e)
    go e@(TextToUpper      {}) = Left (UnsupportedStatement e)
    go e@(TextUnpack       {}) = Left (UnsupportedStatement e)
    go e@(TextPack         {}) = Left (UnsupportedStatement e)
    go e@(TextToList       {}) = Left (UnsupportedStatement e)
    go e@(TextLength       {}) = Left (UnsupportedStatement e)
    go e@(TextCompare       {}) = Left (UnsupportedStatement e)
    ...
    go e@(ListPermutations   ) = Left (UnsupportedStatement e)
    go e@(ListChoose         ) = Left (UnsupportedStatement e)
    go e@(ListFixed          ) = Left (UnsupportedStatement e)
    go e@(ListFixedFromList  ) = Left (UnsupportedStatement e)
    go e@(ListFixedToList    ) = Left (UnsupportedStatement e)
    go e@(ListFixedHead      ) = Left (UnsupportedStatement e)
    go e@(ListFixedTail      ) = Left (UnsupportedStatement e)

    go e@(Sym _                ) = Left (UnsupportedStatement e)
    go e@(SymLit _            ) = Left (UnsupportedStatement e)
    go e@(SymFromText      ) = Left (UnsupportedStatement e)
    go e@(SymToText        ) = Left (UnsupportedStatement e)

    go e@(SZ                 ) = Left (UnsupportedStatement e)
    go e@(SS                 ) = Left (UnsupportedStatement e)
    go e@(SZLit              ) = Left (UnsupportedStatement e)
    go e@(SSLit            {}) = Left (UnsupportedStatement e)
    go e@(SSFromNonZero      ) = Left (UnsupportedStatement e)
    go e@(SSToNonZero        ) = Left (UnsupportedStatement e)
    go e@(SSToNatural        ) = Left (UnsupportedStatement e)
    go e@(SSAdd              ) = Left (UnsupportedStatement e)
    go e@(SSSubtract         ) = Left (UnsupportedStatement e)
    go e@(SSEqual            ) = Left (UnsupportedStatement e)
    go e@(SSLessThan         ) = Left (UnsupportedStatement e)

    go e@(Proof              ) = Left (UnsupportedStatement e)
    go e@(ProofLit           ) = Left (UnsupportedStatement e)
    go e@(ProofToBool        ) = Left (UnsupportedStatement e)
    go e@(ProofFromBool      ) = Left (UnsupportedStatement e)

    go e@(PVoid              ) = Left (UnsupportedStatement e)
    go e@(PVoidUndefined        ) = Left (UnsupportedStatement e)
    go e@(PVoidError        ) = Left (UnsupportedStatement e)
    go e@(PVoidKindUndefined        ) = Left (UnsupportedStatement e)
...
dhallToExpression expr0 = go (Dhall.Core.normalize expr0)
  where
    ...
    go (NonZeroLit (NonZeroP a)) = do
        go (IntegerLit (fromIntegral a))
    go (DateTimeLit (DateTimeC a)) = do
        go (IntegerLit (round (realToFrac a :: Double)))
    go (RegexLit (RegexC a)) = do
